...
import Animated, { cancelAnimation, interpolate, runOnJS, useDerivedValue, useSharedValue, withSpring } from "react-native-reanimated";
...
export function PanResponderAdapter({
      ...
      panX.value = withSpring(offset, DefaultTransitionSpec, finished => {
        if (finished) {
          ...
          if (onTabSelectRef.current) {
            runOnJS(onTabSelectRef.current)({
              index
            });
          }
          runOnJS(() => {
  ...
  const translateX = useDerivedValue(() => {
    const interpolatedValue = interpolate(panX.value, [-maxTranslate, 0], [-maxTranslate, 0], "clamp");
